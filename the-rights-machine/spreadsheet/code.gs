...
function onOpen() {
  ...
  var menuEntries = [
    {name: "Check", functionName: "availability"}
  ];
  ss.addMenu("Availability", menuEntries);
}
...
function availability(e) {
  var queries = getQueriesJson(e);
  var items = getItemsJson(e);
  var task = { "task": { "queries": queries, "items": items } };
  var json = makeJSON_(task, getExportOptions(e));
  ...
  //displayText_(httpResponse.getContentText());
}

function outputResult(source) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheets = ss.getSheets();
  var sheet = sheets[2];

  var values = [];
  var headers = [ "name", "available?", "date checked"];
  values.push(headers);

  var responses = source["responses"];
  responses.forEach(function(object) {
    var row = [];
    row.push(object["name"]);
    row.push(object["available"]);
    row.push(object["created"]);
    values.push(row);
  });

  var range = sheet.getRange("A1:C" + values.length);
  range.setValues(values);
}

function getQueriesJson(e) {

  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheets = ss.getSheets();
  var sheet = sheets[1];

  var rowsData = getItems(sheet, getExportOptions(e));
  var queries = [];

    rowsData.forEach(function(object) {

      var components = [];
      const {  start, end, ...rest } = object;

      var start_str = new Date(start).toISOString().slice(0,10);
      var end_str = new Date(end).toISOString().slice(0,10);
      ...
      for (const property in rest) {
          ...
          var component = { "name": property, "elements" : [] };
          var csv = object[property];
          if(csv)
          {
            var t = csv.split(',');
            for(let e of t)
            {
              var element = { "value" : e.trim() };
...
function getItemsJson(e) {
  ...
  var rowsData = getItems(sheet, getExportOptions(e));
  var itemNames = getItemsNames(rowsData);

  const map1 = new Map();
  var items = [];
  var index = 0;
  for (let name of itemNames) {
    var item = { "name": name, "lines": [] };
    items.push(item);
    map1.set(name, index);
    ++index;
  };
  ...
  rowsData.forEach(function(object) {
      ...
      var components = [];
      const {  name, start, end, exclusivity, type, ...rest } = object;
      ...
      var idx = map1.get(object["name"]);
      items[idx].lines.push(line);
  });

  return items;
}

function getItems(sheet, options) {
...
function getRowByName(rows, name) {
  var result = [];
    rows.forEach(function(object) {
      if(object["name"] == name) result.push(object["name"]);
  });
  return result;
...
function displayText_(text) {
  ...
  SpreadsheetApp.getUi()
      .showModalDialog(output, 'Availability Report');
}
